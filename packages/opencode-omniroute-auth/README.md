...
```bash
bun add @auron/opencode-omniroute-auth
```
...
```typescript
import { clearModelCache } from '@auron/opencode-omniroute-auth/runtime';
...
|--------|------|----------|-------------|
| `plugin` | string[] | No | Plugin packages to load (use `@auron/opencode-omniroute-auth` when installed from Bun or npm) |
...
} from "@auron/opencode-omniroute-auth";
...
} from '@auron/opencode-omniroute-auth/runtime';
...
4. If you use the package programmatically, call `clearModelCache()` from `@auron/opencode-omniroute-auth/runtime`
...
1. Ensure you are using `@auron/opencode-omniroute-auth@1.0.1` or newer
2. Confirm your config includes `"plugin": ["@auron/opencode-omniroute-auth"]`
