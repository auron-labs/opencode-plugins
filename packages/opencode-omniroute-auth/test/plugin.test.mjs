...
test('config hook applies defaults and normalized apiMode without setting provider.api', async () => {
  ...
  assert.equal(config.provider.omniroute.api, undefined);
...
test('config hook normalizes chat completions endpoint baseURL to api root', async () => {
  const plugin = await OmniRouteAuthPlugin({});
  const config = {
    provider: {
      omniroute: {
        options: {
          baseURL: `${getDummyBaseUrl()}/chat/completions`,
          apiMode: 'chat',
        },
      },
    },
...
test('config hook avoids top-level provider.api', async () => {
  const plugin = await OmniRouteAuthPlugin({});
  const config = {
    provider: {
      omniroute: {
  ...
  assert.equal(config.provider.omniroute.api, undefined);
  assert.equal(config.provider.omniroute.options.apiMode, 'responses');
  assert.equal(config.provider.omniroute.options.baseURL, 'http://localhost:20128/v1');
});

test('config hook applies modelList to default fallback models without auth', async () => {
  const plugin = await OmniRouteAuthPlugin({});
  const config = {
    provider: {
      omniroute: {
  ...
  assert.deepEqual(Object.keys(config.provider.omniroute.models), ['gpt-4o-mini']);
  assert.equal(config.provider.omniroute.models['gpt-4o-mini'].name, 'Mini Only');
});
...
test('provider hook preserves modelList regex filters and aliases after config hook option clone', async () => {
  const plugin = await OmniRouteAuthPlugin({});
  ...
  assert.equal(result['openrouter/z-ai/glm-5.2'], undefined);
  assert.equal(result['mimo/mimo-v2.5-pro'], undefined);
  assert.equal(
    result['openrouter/moonshotai/kimi-k2.6'].name,
...
test('provider hook keeps alias duplicates when modelList.dedupe is false', async () => {
  const plugin = await OmniRouteAuthPlugin({});
  ...
  const result = await plugin.provider.models(
    {
      ...
      source: 'config',
      env: [],
      options: { baseURL: getDummyBaseUrl(20135), apiMode: 'chat', modelList: { dedupe: false } },
      models: {},
    },
    { auth: { type: 'api', key: 'test-key' } },
  );

  assert.ok(result['mimo/mimo-v2.5-pro']);
  assert.ok(result['xiaomi-mimo/mimo-v2.5-pro']);
});
