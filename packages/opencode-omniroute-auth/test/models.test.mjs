...
import { calculateLowestCommonCapabilities, clearModelsDevCache } from '../dist/src/models-dev.js';
...
test('fetchModels uses models.dev name for slug-like model names', async () => {
  global.fetch = async (input) => {
    const url = input instanceof Request ? input.url : input.toString();
    if (url.includes('/v1/models')) {
      return new Response(JSON.stringify({
        object: 'list',
        data: [
          { id: 'openrouter/openai/gpt-5-image-mini', name: 'openrouter/openai/gpt-5-image-mini' },
        ],
      }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
  ...
  const models = await fetchModels(
    {
      ...CONFIG,
      modelsDev: {
        url: 'http://localhost:99999/models-dev.json',
      },
    },
    CONFIG.apiKey,
    true,
  );
  assert.equal(models[0].name, 'OpenAI GPT-5 Image Mini');
});

test('fetchModels enriches custom uma models via built-in models.dev provider alias', async () => {
  global.fetch = async (input) => {
    const url = input instanceof Request ? input.url : input.toString();
    if (url.includes('/v1/models')) {
      return new Response(JSON.stringify({
        ...
        data: [
          {
            id: 'uma/umans-coder',
            name: 'umans-coder',
            owned_by: 'openai-compatible-chat-0073c142-d0d4-4faf-b164-8086eeb6bb00',
            root: 'umans-coder',
            parent: null,
          },
        ],
  ...
  assert.equal(model.name, 'Umans Coder');
...
test('fetchModels cleanNames uses resolved provider label for synthetic compatible owners', async () => {
  global.fetch = async (input) => {
    const url = input instanceof Request ? input.url : input.toString();
    if (url.includes('/v1/models')) {
      return new Response(JSON.stringify({
  ...
  assert.equal(model.name, 'Umans AI Coding Plan: Umans Coder');
});

test('fetchModels filters synthetic openai-compatible uuid models', async () => {
  global.fetch = async (input) => {
    const url = input instanceof Request ? input.url : input.toString();
    if (url.includes('/v1/models')) {
      return new Response(JSON.stringify({
        object: 'list',
        data: [
          { id: 'openai-compatible-chat-0073c142-d0d4-4faf-9d95-91ebcbec0d0a' },
          { id: 'openrouter/openai/gpt-5-image-mini', name: 'openrouter/openai/gpt-5-image-mini' },
        ],
      }), {
        status: 200,
  ...
  const models = await fetchModels(CONFIG, CONFIG.apiKey, true);
  assert.deepEqual(models.map((model) => model.id), ['openrouter/openai/gpt-5-image-mini']);
});
...
test('fetchModels uses different cache for different modelList configs', async () => {
  let calls = 0;
  global.fetch = async (input) => {
    const url = input instanceof Request ? input.url : input.toString();
    if (url.includes('/v1/models')) {
  ...
  const config1 = { ...CONFIG, modelList: { cleanNames: true, sort: 'name' } };
  const config2 = { ...CONFIG, modelList: { cleanNames: true, aliases: { model: 'MODEL' }, sort: 'name' } };

  await fetchModels(config1, CONFIG.apiKey, false);
  await fetchModels(config2, CONFIG.apiKey, false);

  assert.equal(calls, 2, 'Should fetch twice for different modelList configs');
});
...
test('deduplication keeps primary root model when alias entry has parent', async () => {
  global.fetch = async (input) => {
    const url = input instanceof Request ? input.url : input.toString();
    if (url.includes('/v1/models')) {
      return new Response(
  ...
  assert.equal(models.length, 1, 'Should keep only the primary model');
  assert.equal(models[0].id, 'mimo/mimo-v2.5-pro');
  assert.equal(models[0].contextWindow, 1048576, 'Should keep primary model metadata');
});

test('fetchModels keeps alias duplicates when modelList.dedupe is false', async () => {
  global.fetch = async (input) => {
    const url = input instanceof Request ? input.url : input.toString();
    if (url.includes('/v1/models')) {
      return new Response(
...
test('fetchModels cleans names, applies aliases, filters, and sorts via modelList config', async () => {
  global.fetch = async (input) => {
    const url = input instanceof Request ? input.url : input.toString();
    if (url.includes('/v1/models')) {
      return new Response(
  ...
  assert.equal(models.find((model) => model.id === 'openrouter/z-ai/glm-5.2'), undefined);
});
